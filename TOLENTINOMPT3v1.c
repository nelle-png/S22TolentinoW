/////
/*
	FIXED RANDOM WORDS INSTEAD OF LETTERS
	
	NEEDED AS OF JULY 18, 2025 
	- In the process of changing the random letter to a random word
	- Working on adding bonus features (color, sorting options)
...
// this will pull a word and jumble it for the user to guess soon
void jumbledWord(gameData wordlist[], int entryCount, char *originalWord, char *jumbledWord){
	int random; // for index
	int length;
	int i;
	int j;
	char temp; // for the jumbled character
	
	if(entryCount > 0){
		random = rand() % entryCount;
		strcpy(jumbledWord, wordlist[random].strWord);
		length =  strlen(jumbledWord);
		
		for(i = 0; i < length; i++){
			j = rand() % length;
			
			// swaps letters to **really** jumble the words in wordlist
			temp = jumbledWord[i];
			jumbledWord[i] = jumbledWord[j];
			jumbledWord[j] = temp;
		}
	} else {
		jumbledWord[0] = '\0'; // initialize to empty if no words are detected	
	}
        ...
        while (player->nLives > 0) {
           ...
           // Game status
            if (validWord == 0) {
                ...
                jumbleRandomWord(wordlist, *entryCount, jumbledWord);
